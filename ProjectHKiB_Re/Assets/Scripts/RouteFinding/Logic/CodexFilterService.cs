...
public static class CodexFilterService
{
    ...
    // 두 키워드 배열이 하나라도 겹치는지(대소문자 무시, trim) — GroupByKeyword와 같은 비교 규칙을
    // 쌍(pair) 단위로 쓰고 싶을 때(예: 노트 그래프의 "연관 단서" 자동 간선) 이걸 재사용한다. 이 규칙을
    // View 쪽에서 따로 구현하면 나중에 대소문자/trim 처리가 바뀔 때 여기와 어긋날 수 있다.
    public static bool SharesKeyword(string[] a, string[] b)
    {
