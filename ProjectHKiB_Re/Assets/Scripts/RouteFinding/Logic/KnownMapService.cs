...
public static class KnownMapService
{
    ...
    // 이 노드에 딸린 단서 중 아직 획득 못 한 개수 — 도감의 "미발견 단서 자리 표시"(Clue_System.md
    // 6-2)가 슬롯 개수를 정할 때 쓴다(2026-07-26, CodexPanel.BuildUnacquiredPlaceholders에서 이동).
    public static int CountUnacquiredClues(MapNodeData node, RouteProgressState progress)
    {
        if (node.clueIds == null) return 0;
        return node.clueIds.Count(id => !progress.IsClueAcquired(id));
    }
}
