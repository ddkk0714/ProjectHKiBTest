...
//
// Data/에 있지만 Codex(도감)/Note(노트) UI와 Logic/CodexFilterService가 공유하는 서비스
// 계층 DTO다(NoteSystem_기획서.md — Note·Codex는 데이터·서비스 레이어만 공유, 모듈·UI는
// 분리) — 원래 Codex/(UI 폴더)에 있었는데, Logic/CodexFilterService.cs가 UI 폴더를 역참조하는
// 모양이 되어 있어서 여기로 옮겼다(2026-07-26). UnityEngine 의존 없는 순수 데이터라 위치만
// 옮기면 되고 View 쪽 코드는 수정할 필요 없다(전부 global namespace라 using 없이 그대로 참조됨).
