...
//
// ▸ StateMachine → 이벤트 연동 (StateMachine/Actions/Event/SetRouteEventFlagAction, 2026-07-26 신규)
//   위 SetEventFlag를 이벤트 그래프/전투 State 어디서든 코드 없이 StateAction 배선만으로 호출할
//   수 있게 하는 래퍼. mapGuid를 비워두면 RouteModule.Instance.CurrentLocation을 자동 사용한다.
//   현재 감정 벡터 모듈의 잠/황홀 축 진입 State에 이미 배선되어 있어(Enemy_Rusher_SleepState/
//   EcstasyState, Delta_Base_Sleep/Ecstasy_StartState), 아래 이벤트 키가 항상 발행 가능한 상태다
//   — ClueData.requiredEventKey를 이 값으로 지정하면 별도 코드 없이 감정 트리거만으로 단서 공개:
//     playerSlept / playerEcstasy : 플레이어(Delta_Base) 잠/황홀 진입
//     enemySlept  / enemyEcstasy  : Rusher 잠/황홀 진입
//   상세는 EmotionVectorModule.cs 상단, SetRouteEventFlagAction.cs 참고.
//
...
public class RouteModule : MonoBehaviour, IEventSaveProvider
{
    ...
    public Dictionary<string, int> EventFlags => Progress?.BuildEventFlagsSnapshot()
        .ToDictionary(kv => kv.Key, kv => kv.Value ? 1 : 0) ?? new Dictionary<string, int>();
    public void SetEventFlag(string id, int value) => Progress?.ApplyEventFlag(id, value != 0);
