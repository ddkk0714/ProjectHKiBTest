...
namespace RouteFinding.MapView
{
    ...
    public class MapNodeView : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        private Button            _btn;
        private TextMeshProUGUI   _diffLabel; // 2026-07-14 추가 — 난이도/통과 불가 표시(원래 MapEdgeView가 갖던 것)
        ...
        private static readonly Color ColLocked   = new(0.55f, 0.18f, 0.55f);    // 필수 장비 미충족 — 진입 불가
        private static readonly Color ColLabelDefault = new(1f, 1f, 1f, 0.88f);

        // visualImage: 실제로 색이 칠해지는 자식 그래픽(MapViewer의 "Visual" 자식) — 이 GO 자신의
        //   Image는 클릭 판정 전용 투명 배경이라 색상 표시에는 쓰지 않는다.
        // diffLabel: MapViewer가 외부에서 주입하는, 회전 없이 고정된 별도 GO(노드 근처에 배치).
        public void Init(MapNodeData data, Image visualImage, TextMeshProUGUI diffLabel = null)
        {
            Data       = data;
            _bg        = visualImage;
        ...
        //
        // 2026-07-14 추가: isPassable/requiredGears/difficulty/cleared — 원래 MapEdgeView가 갖던
        // "통과 불가(필수 장비 미충족)" + 난이도 숫자 표시가 전투의 연결→맵 이동에 따라 여기로 옮겨왔다.
        public void SetState(bool visited, bool hasClue, bool isStart, bool isSelected, bool isOnPath, bool isOnBlockedPath, bool known,
            bool isPassable = true, EmotionColor[] requiredGears = null, float difficulty = 0f, bool cleared = false)
        {
            if (_bg == null)
            {
                Debug.LogWarning($"[MapNodeView.SetState] {name}: _bg가 null! Init()에 visualImage가 전달되지 않았습니다.");
                return;
            }
            ...
            Color col = (isSelected, isOnPath, isOnBlockedPath, locked, isStart, visited, hasClue) switch
            {
                (true, _, _, _, _, _, _) => ColSelected,
                (_, true, _, _, _, _, _) => ColOnPath,
                (_, _, true, _, _, _, _) => ColBlockedPath,
                (_, _, _, true, _, _, _) => ColLocked,
                (_, _, _, _, true, _, _) => ColStart,
                (_, _, _, _, _, true, _) => ColVisited,
                (_, _, _, _, _, _, true) => ColHasClue,
                _                        => ColNoClue,
            };
            _bg.color = col;
